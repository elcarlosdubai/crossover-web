...
import { updateSlide } from "@/app/actions/portada";
...
export default function EditForm({ initialData }: { initialData: any }) {
  ...
  const [formData, setFormData] = useState({
    bg: initialData.foto_portada || "",
    tag: initialData.galeria_urls?.[0] || "",
    title: initialData.titulo || "",
    subtitle: initialData.slug || "",
    description: initialData.contenido_html || "", btnText: initialData.galeria_urls?.[1] || "", btnLink: initialData.galeria_urls?.[2] || "", btn2Text: initialData.galeria_urls?.[3] || "", btn2Link: initialData.galeria_urls?.[4] || ""
  });

  const handleGuardar = async () => {
    setIsSaving(true);
    const res = await updateSlide(initialData.id, formData);
    setIsSaving(false);
    if (res.success) {
  ...
  return (
    <div className="max-w-4xl mx-auto pb-12">
      ...
      <h1 className="text-3xl font-display font-black text-gray-900 mb-8 tracking-tight">Editar Imagen del Carrusel</h1>
      ...
      <div className="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm flex flex-col gap-6">
        ...
        <div>
          ...
          <input type="text" value={formData.tag} onChange={(e) => setFormData({...formData, tag: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl py-3 px-4 focus:border-blue-500 outline-none" />
        </div>
        <div>
          <label className="block text-sm font-bold text-gray-900 mb-2">Título Gigante 1</label>
          <input type="text" value={formData.title} onChange={(e) => setFormData({...formData, title: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl py-3 px-4 focus:border-blue-500 outline-none uppercase" />
        </div>
        <div>
          <label className="block text-sm font-bold text-gray-900 mb-2">Título Gigante 2</label>
          <input type="text" value={formData.subtitle} onChange={(e) => setFormData({...formData, subtitle: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl py-3 px-4 focus:border-blue-500 outline-none uppercase" />
        </div>
        <div>
          <label className="block text-sm font-bold text-gray-900 mb-2">Descripción</label>
          <input type="text" value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl py-3 px-4 focus:border-blue-500 outline-none" />
        </div>
        ...
        <button onClick={handleGuardar} disabled={isSaving} className="mt-4 bg-emerald-600 text-white py-4 px-8 rounded-xl font-bold uppercase tracking-widest text-sm hover:bg-emerald-700 transition-colors w-full flex justify-center items-center gap-2">
          <span className="material-symbols-outlined">{isSaving ? 'sync' : 'save'}</span> {isSaving ? 'Guardar Cambios' : 'Guardar Cambios'}
        </button>
      </div>
    </div>
  );
