...
import { deleteSlide } from "@/app/actions/portada";
...
export function SearchBar({ initialQuery }: { initialQuery: string }) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    timeoutRef.current = setTimeout(() => {
      if (val.trim()) {
        router.push(`/admin/dashboard/portada?query=${encodeURIComponent(val)}`);
        router.refresh();
      } else {
...
export function ActionButtons({ id, title }: { id: number, title: string }) {
  ...
  const handleDelete = async () => {
    const confirm = window.confirm(`¿Seguro que deseas eliminar la imagen del carrusel: "${title}"?`);
    if (!confirm) return;
    setIsDeleting(true);
  ...
  return (
    <div className="flex items-center justify-end gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
      <Link href={`/admin/dashboard/portada/${id}`} className="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center text-gray-600 hover:bg-blue-100 hover:text-blue-600 transition-colors" title="Editar">
